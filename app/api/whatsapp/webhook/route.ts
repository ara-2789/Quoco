...
export async function handleWebhookPost(
  ...
  // TEMPORARY DIAGNOSTIC, RE-ADDED 2026-09-03, TO BE REVERTED THE SAME DAY --
  // ad-hoc-menu tap-test payload capture, second attempt (first attempt's
  // sends never reached the webhook at all -- wrong sender, error 63015 --
  // so this never fired; reverted and rebuilt identically once the sender
  // was fixed). Gated to the one test sender number so this never runs for
  // real traffic. Never logs params.Body or params.From -- only the full
  // set of KEY NAMES present (safe: no phone numbers, no message content)
  // plus the VALUES of any key that looks like an interactive-reply field
  // (matches /button|list/i in its name), since that's the one thing this
  // capture exists to see and neither Body nor From can match that pattern.
