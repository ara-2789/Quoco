...
describe('clearMessagingBlock (BOT-27 clear-half)', () => {
  ...
  it('clears messaging_blocked for the matching engineer', async () => {
    await setBlocked(true)
    expect(await readBlocked()).toBe(true)

    const { error } = await clearMessagingBlock(testClient(), testEngineerId(), TEST_TENANT_ID)
    expect(error).toBeNull()
    expect(await readBlocked()).toBe(false)
  })

  it('is tenant-scoped: a wrong tenant_id does NOT clear the flag', async () => {
    await setBlocked(true)
    expect(await readBlocked()).toBe(true)

    // Well-formed but non-matching tenant — the UPDATE matches zero rows (no
    // error), so the flag must remain true.
    const wrongTenant = '00000000-0000-4000-a000-0000000000ff'
    const { error } = await clearMessagingBlock(testClient(), testEngineerId(), wrongTenant)
    expect(error).toBeNull()
    expect(await readBlocked()).toBe(true)
  })
})
