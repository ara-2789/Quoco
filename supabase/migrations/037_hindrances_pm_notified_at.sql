...
-- probe (pm_notified_at nullable/timestamptz/no default, comment intact
-- and untruncated including the race paragraph -- full probe record in
-- the PR that moved this file from docs/reviews/ into this directory).
-- Struck through below, not rewritten -- the WRITTEN/REHEARSED facts
-- still hold, only the applied-status and file-location claims are
-- superseded: ~~NOT YET APPLIED. Per CLAUDE.md's "a migration file enters
-- supabase/migrations/ when it is being applied, not when it is written"
-- rule, this file lives in docs/reviews/ until an apply is actually
-- happening.~~ This file now lives in `supabase/migrations/` because that
-- apply has happened.
...
-- Ledger untouched throughout (at rehearsal time).
-- ~~NOT YET APPLIED ANYWHERE.~~ DATED CORRECTION (2026-09-06): APPLIED TO
-- PROD, ledger repaired (`supabase migration repair --status applied
-- 037 --linked`), verified by catalog probe -- structure and the full
-- COMMENT (including the race paragraph) confirmed intact and
-- untruncated.
-- =============================================================================
