...
-- DATED CORRECTION (2026-09-06): APPLIED TO PROD, verified by catalog
-- probe (structure, both constraints, all three comments intact and
-- untruncated -- full probe record in the PR that moved this file from
-- docs/reviews/ into this directory). Struck through below, not
-- rewritten, per this project's own correction discipline -- the
-- WRITTEN/REHEARSED facts still hold, only the applied-status claim is
-- superseded: ~~NOT YET APPLIED.~~
-- WRITTEN, REHEARSED A THIRD TIME (2026-09-05, against the current file --
-- see the REHEARSED status near the end for the full record). The first
-- two rehearsals ran against an earlier version of this
...
-- `f878057d...` pair (named near this file's own top) as current.
-- ~~NOT YET APPLIED TO PROD.~~ DATED CORRECTION (2026-09-06): APPLIED TO
-- PROD, ledger repaired (`supabase migration repair --status applied
-- 036 --linked`), verified by catalog probe -- structure, both
-- constraints (including the pairing CHECK's exact NULL semantics), and
-- all three comments confirmed intact and untruncated. This file now
-- lives in `supabase/migrations/` because it is applied, not merely
-- written -- see CLAUDE.md's own "a migration file enters
-- supabase/migrations/ when it is being applied" rule.
-- =============================================================================
