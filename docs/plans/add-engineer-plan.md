# Add-engineer screen — build plan (rev7)

> PLAN ONLY. rev1 `c86c5b6` (2026-09-18 23:06 IST), rev2 `e67e297` (2026-09-18 23:41 IST), rev3 `9b9187c` (2026-09-19 00:15 IST), rev4 `dba9cfc`, rev5 `fad98e3`, rev6 `eff12a0` — each pinned
> (`git show <sha>:docs/plans/add-engineer-plan.md`). rev7 written 2026-09-19 (IST) on `feat/add-engineer-plan`, base `origin/main` @ `0744110` (re-fetched; unchanged).
> No application code, no migration file, no SQL intended to ship. Tier: FULL.
>
> **Evidence rule.** Every claim cites source printed in `~/Desktop/add-engineer-plan-rev7.txt` ("the log"). Citations are `path:line`; database claims cite `probe <letter>` (rev7 adds probes `pg_1`–`pg_7`, named as such); throwaway scripts
...
> D12 (rev6) Option D — the partial unique index leaves this slice; no test fixture is reshaped; **D13 (rev7) — in this slice, dated, not a status filter (§4.9).**
>
...
> **Deploy-order hazard, new in rev7 (§4.9):** the board query will select `users.deactivated_at`; if the app ships before migration 048 is applied, Daily Logs and Today both render their error state (probe pg_7).
...
## Dated corrections, 19 Sep 2026 (rev7) — every change in this pass
...
- **F5 — NEW (rev7), NAMED, NOT FIXED, OUTSIDE THE THREE SURFACES: two DPR-side reads use `users.status` as a plain current-state filter — the same wrong-rule class §4.9 rejects for the board.** (a) `lib/dpr/accountability.ts:155-160` builds the accountability roster with `.eq('users.status','active')` **and no date comparison**, though the function takes a `log_date` (`:153`). (b) `lib/dpr/dispatch.ts:418` computes `stillActiveMember = membership !== null && engineer?.status === 'active'` and, for a log day on which real data exists but the engineer is not active, labels an un-owed half `left_early` (`:436-438`). **Reading, not run:** if a DPR is (re)generated for a **past** date after an engineer has been deactivated, (a) omits that engineer from that day's roster and (b) may label their halves "left this project during the day" for a day on which they were an active member for all of it. Whether a past-date regeneration path exists is **UNKNOWN** (#23). The same file family already has a date-aware precedent on the *join* side: `dispatch.ts:427-428` compares `created.date === payload.log_date` where `created = istParts(new Date(membership.created_at))` ("joined this project today"). §4.9's `deactivated_at` boundary is the missing *leave* side. **Not in scope; recorded so the future fix has the pointer.**
...
`isAfterDeactivation(status, deactivatedAt, logDate) := status === 'deactivated' && deactivatedAt !== null && logDate >= istDateString(new Date(deactivatedAt))`
...
- **`status === 'deactivated'`, not `!== 'active'`.** `users_status_check` allows **three** values — `'pending'`, `'active'`, `'deactivated'` (probe pg_4) — so `<> 'active'` would sweep `pending` users in. The guard also means a reactivated engineer (status back to `'active'`) is always shown, whatever stale `deactivated_at` remains. **No reactivation path is designed in this slice**; if one is built, the days between deactivation and reactivation will show as missing (one slot, no history — D10's limit; UNKNOWNS #24).
...
|---|---|---|
| `lib/daily-logs/query.ts` | `getDailyLogsBoard`, roster select `:161` | add `status, deactivated_at` to `users(id, full_name, messaging_blocked, whatsapp_number)`; `RosterRow.users` (`:92-100`) gains the two fields |
...
| same | new exported pure `isAfterDeactivation` and `collectMissingCheckins` | the second is the Today loop `dashboard/page.tsx:140-174` **moved verbatim** plus one line, `if (e.afterDeactivation) continue`, so the count is testable (T25); kept in `query.ts` to stay inside the granted scope |
...
**Deploy order — a lockstep, not a detail.** The new select reads `users.deactivated_at`. Against a database without it PostgREST rejects the whole query (probe pg_7: `42703: column "deactivated_at" does not exist`); the board then returns `{ status: 'error' }` (`query.ts:173`, `reportReadFailure`) and **both** Daily Logs (`daily-logs/page.tsx:101-102`) and Today (`dashboard/page.tsx:128-130`) render their error state. Therefore **048 must be applied to test-db (CI) and to prod before this app change deploys**. The DOWN (drops the columns) needs the app change rolled back **first** — the DOWN-block rule in `CLAUDE.md` §7: "a DB rollback and an app rollback are not the same operation" — and the DOWN rehearsal must confirm the board still loads after the DOWN with the app at the *previous* commit. **No new GRANT is needed:** `authenticated` holds table-level `SELECT` on `users` (probe pg_6: 17 of 17 columns) and RLS `users_select` still bounds the rows (probe j; not executed as a PM). The new columns follow the table's existing grants (`anon` also holds table-level `SELECT` — pre-existing, RLS-bounded).
...
- **Deploy order (rev7, §4.9):** the D13 app change selects `users.deactivated_at`, so **048 lands on test-db and prod BEFORE the app change deploys** (probe pg_7: without the column the board query fails `42703` and Daily Logs and Today both show their error state). The DOWN needs the app change rolled back first; its rehearsal must confirm the board still loads after the DOWN with the app at the previous commit.
...
**Not touched by this plan or the slice:** anything under `app/api/whatsapp/`, `lib/whatsapp/`, `lib/auth/`, or any existing migration; **under `lib/daily-logs/`, only `query.ts`** (D13); `is-project-pm.ts` and `normalise.ts` are imported, not edited. **This revision's diff:** exactly `docs/plans/add-engineer-plan.md`.
...
23. **Whether a past-date DPR regeneration path exists** (F5): if it does, `accountability.ts:155-160` and `dispatch.ts:418,436-438` would treat a since-deactivated engineer as absent/left-early for a day they worked. Read, not run.
24. **Reactivation.** No path is designed; the rule's `status === 'deactivated'` guard makes an active engineer always visible, but the gap days would show as missing, and D10's one-slot `deactivated_*` would be stale on re-deactivation (`already_deactivated` applies only while `status='deactivated'`).
...
**Decisions still open:** ~~**D13** (product; not blocking)~~ — settled (rev7). **Flagged for confirmation:** **D14** (i)/(ii) (not blocking).
