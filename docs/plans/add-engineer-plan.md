# Add-engineer screen — build plan, SLICE 1 of 2 (rev10)
...
>
> **What this document contains.** The add screen (paste → preview → confirm → apply); the dry-run flag (D7); the `add_engineers_to_project` SECURITY DEFINER function and the **shared authorisation helper**; `users.registered_by`, `users.registered_at`, `users.consent_attested`; the six-role CHECK on `project_members.role`; the S1 lint extension; phone validation and normalisation (§3); the boundary-test literal (§7.3); the strings those need (§9); the D12 decision and its rider; the residual-risk text; the **deploy-order rule with merge as the event, its rollback order and the Preview-database finding** (§6.1). Decisions here: D1–D9, D11, D12, D16. Findings: N1, the S1 lint, the D12 rider. Tests: T1–T22 (T3, T4, T22 struck) and T39.
>
> **What it explicitly does NOT contain.** Deactivate, reactivate, the `engineer_episodes` table and its index and RLS, the D13 dated board rule, the half-status / evening-chip change (Edge 1), the classifier's `deactivated_on_this_project` outcome (Edge 2), the unreachability report, and D10, D13–D15, D17–D20 with F1–F5 — **all of that is slice 2.**
>
> **Dependency.** Slice 1 stands alone. **Slice 2 depends on slice 1** — on its `users` columns (`registered_by`, `registered_at`, `consent_attested`), on the shared authorisation helper (§2.7) and on `add_engineers_to_project` (which slice 2 redefines, see slice 2). Slice 1 ships first.
>
...
*Slice 1 note (rev10): in slice 1 the app-side dependencies on 048 are the `add_engineers_to_project` RPC and the three `users` columns — an app merged before 048 fails on the missing function, the same class as the board example below. The sentences about the board and `engineer_episodes` describe slice 2's dependency and are kept verbatim.*
...
**No slice-1 string may imply that a removal or undo path exists (rev10).** Slice 1 ships with no deactivate, no undo and no way to free a number (see the top of this document), so **no constant, formatter or error text here may say or suggest that an engineer can be removed, switched off, undone or "fixed later from the dashboard".** This binds the wording author for, at least, `ADD_ENGINEERS_PAGE_INTRO`, `ADD_ENGINEERS_FORMAT_HELP`, `ADD_CONSENT_ATTESTATION`, the confirm text (`ADD_CONFIRM`), `RESULT_ROW_ADDED`, `ENGINEERS_LIST_TITLE`, and every rejection text (a rejected number is *not registered*; nothing is undone). `ADD_ENGINEERS_EDIT_BUTTON` edits the **pasted text before apply** — it must not read as editing a registered engineer.
...
45. **Whether slice 1 includes a read-only engineers list page** (`ENGINEERS_LIST_TITLE`, `ENGINEER_STATUS_ACTIVE`, `formatRegisteredLine` stayed in slice 1 §9 by content; the list page with per-engineer controls was written for deactivate). Placement **ASSUMED**, not decided; the file list (§12) names `projects/[id]/engineers/page.tsx` as ASSUMED.
