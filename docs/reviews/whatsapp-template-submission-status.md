...
**`allow_category_change` IS DEPRECATED AND NON-PROTECTIVE (2026-09-02) —
READ BEFORE THE NEXT BATCH.** `quoco_morning_checkin_v3` (1v3) came back
Meta-approved as MARKETING despite being submitted as UTILITY.
`allow_category_change: true` was found live on ALL FIVE templates checked
(1, 1v3, 2v3, 8v2, 14), including the one that never got recategorised —
this codebase never sets the field explicitly, it's a Twilio/Meta default.
Per Twilio's own changelog, Meta has discontinued this property:
reclassification-to-MARKETING is now the default with no way to prevent it
through this field. **Do not set `allow_category_change: false` on a future
resubmission expecting it to guard against recategorisation — it won't.**
Full investigation, options considered, and the decision:
...
| 14 | `quoco_dpr_owner_no_report` | Utility | primary (NEW, 2026-08-31, §37(d)) | 2026-08-31 | approved | First owner-facing WhatsApp template in the product. Second sentence is load-bearing (`whatsapp-templates.md`'s own note) — without it the body ends on `{{2}}`, an automatic-rejection condition. Compliance audit: 6/6 pass. HX SID `HX20e017592136f4fa6fdaf6cb200bf377`, approval request status `approved` (checked 2026-09-02), category UTILITY as submitted. **Approved body (HX20e017592136f4fa6fdaf6cb200bf377):** `No site report was received for {{1}}, dated {{2}}. There is nothing to share for this date.` — this exact line is what `test/unit/owner-no-report.test.ts`'s copy-drift test reads and compares against `OWNER_NO_REPORT_TEMPLATE_BODY` (`lib/dpr/owner-no-report.ts`), per 034 review package §12c's own argued shape (the recorded-approved body, not `docs/whatsapp-templates.json`'s current draft). |
...
| 1v3 | `quoco_morning_checkin_v3` | Utility (submitted) | re-cut (2026-08-31, not a spare) | 2026-08-31 | approved, category MARKETING | Re-cuts #1 with approved copy from Aravind. Compliance audit: 6/6 pass. `docs/whatsapp-templates.md`'s "Re-cut variants" section has the full copy and audit detail. **Came back Meta-approved as MARKETING, not the submitted UTILITY** (checked 2026-09-02) — full investigation and decision: `docs/reviews/whatsapp-marketing-category-investigation.md`. Decision (Aravind, 2026-09-02): does NOT repoint `templates.ts` — #1's own SID keeps sending live traffic indefinitely unless a recategorisation request (filed in parallel, WhatsApp Manager) succeeds. HX SID `HXbb534f41c814a2c3a32b5682713579df`. |
| 2v3 | `quoco_evening_checkin_v3` | Utility | re-cut (2026-08-31, not a spare) | 2026-08-31 | approved | Re-cuts #2 with approved copy from Aravind; drops `{{3}}` (morning-plan echo) per `design-decisions-beta-feedback.md` §40, which supersedes §28(s). Compliance audit: 6/6 pass. Retires #2b going forward (2b's own approval and SID are untouched — it just stops being the design once the repoint happens). Approved as UTILITY, category unchanged from submission (checked 2026-09-02). **Repointed in `templates.ts`, 2026-09-02** — see `docs/whatsapp-templates.md`'s repointing checklist item 2/3. HX SID `HX8fb39a251eee9bfb2ec075086cd7800a`. |
| 8v2 | `quoco_engineer_optin_v2` | Utility | re-cut (2026-08-31, not a spare) | 2026-08-31 | received | Re-cuts #8 with the false "Reply STOP" promise (§29(a)) removed, per direct instruction. **The STOP line is omitted, not replaced** — checked directly against `docs/plans/adhoc-menu-spec.md`: the ad-hoc menu (§29(b)'s real opt-out route) is decided, not built, so nothing true can be said about an opt-out route yet. Compliance audit: 6/6 pass. Not blocked by GATE 2's literal name match (`quoco_engineer_optin` ≠ `quoco_engineer_optin_v2`); #8 itself remains held under GATE 2, unchanged. **Rejection contingency, recorded before submission (Aravind, 2026-08-31):** this template carries no opt-out language at all — honest, and expected to clear review as business-initiated onboarding, not marketing. **That expectation held**: approved as UTILITY, category unchanged from submission (checked 2026-09-02) — the one template of the four in this batch where the "will this read as marketing" question was asked explicitly before submitting, and it didn't. If Meta rejects it wanting an opt-out line, the fix is building §28(x)/§29(b)'s ad-hoc menu first, then re-cutting the copy again — never inventing a promise ahead of the mechanism that keeps it. Full text: `docs/whatsapp-templates.md`'s 8v2 section. HX SID `HX40923a2de0fa55ea4e7335607c9a1bb9`. |
