...
| 14 | `quoco_dpr_owner_no_report` | Utility | primary (NEW, 2026-08-31, §37(d)) | 2026-08-31 | received | First owner-facing WhatsApp template in the product. Second sentence is load-bearing (`whatsapp-templates.md`'s own note) — without it the body ends on `{{2}}`, an automatic-rejection condition. Compliance audit: 6/6 pass. Dry run only this pass (`npx tsx scripts/submit-templates.ts`, no `--submit`) — Aravind submits, credentials are on his machine. HX SID `HX20e017592136f4fa6fdaf6cb200bf377`, approval request status `received` (submitted 2026-08-31) |
