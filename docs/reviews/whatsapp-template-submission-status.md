...
| 1v3 | `quoco_morning_checkin_v3` | Utility | re-cut (2026-08-31, not a spare) | — | not submitted (dry run only) | Re-cuts #1 with approved copy from Aravind. Compliance audit: 6/6 pass. `docs/whatsapp-templates.md`'s "Re-cut variants" section has the full copy and audit detail. Does not repoint `templates.ts` — #1's own SID keeps sending live traffic until a separate change. |
| 2v3 | `quoco_evening_checkin_v3` | Utility | re-cut (2026-08-31, not a spare) | — | not submitted (dry run only) | Re-cuts #2 with approved copy from Aravind; drops `{{3}}` (morning-plan echo) per `design-decisions-beta-feedback.md` §40, which supersedes §28(s). Compliance audit: 6/6 pass. Retires #2b going forward (2b's own approval and SID are untouched — it just stops being the design once the repoint happens). |
| 8v2 | `quoco_engineer_optin_v2` | Utility | re-cut (2026-08-31, not a spare) | — | **NOT SUBMITTED — draft copy, awaiting Aravind's approval, separate from GATE 2** | Re-cuts #8 with the false "Reply STOP" promise (§29(a)) removed, per direct instruction. **The STOP line is omitted, not replaced** — checked directly against `docs/plans/adhoc-menu-spec.md`: the ad-hoc menu (§29(b)'s real opt-out route) is decided, not built, so nothing true can be said about an opt-out route yet. Compliance audit: 6/6 pass. Not blocked by GATE 2's literal name match (`quoco_engineer_optin` ≠ `quoco_engineer_optin_v2`), but held anyway on its own copy-approval requirement until Aravind signs off. #8 itself remains held under GATE 2, unchanged. |
