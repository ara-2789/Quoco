...
| observation | `public` holds **14** SECURITY DEFINER functions of 16; the plan's probe `n` said 15 (section 13) |
...
### 8.5 State my red runs LEFT on test-db — a side effect of this work, found by the re-check, NOT yet cleaned
...
4. **U-4 — Definer-function count.** The live `public` schema has **14** SECURITY DEFINER functions (of 16); the plan's probe `n` and its "all 15 existing definer functions" say 15. Not reconciled — I do not know which is stale, or whether a function was dropped or counted from another schema.
...
13. **U-13 — test-db is currently not clean, and I caused it.** My red runs stranded run-scoped fixtures (section 8.5); `owner-deliver-job` fails on test-db for **any** run until two rows are deleted. The cause is consistent with all observations but **not confirmed by a rerun** — that needs the cleanup, which needs Aravind. Whether *other* users of test-db (CI on other PRs, other sessions) ran in the window and were affected is unknown.
