...
> **DATED ADDENDUM (2026-09-20).** The statements in this package about *state* were written before the apply and describe the past. **048 has since been applied to test-db only** (`exfccwlrhoutkgrlikod`, 2026-09-20), DOWN-rehearsed there, ledgered, observed, and CI-green at `fd96199`; the migration now lives in `supabase/migrations/`; the runbook was rehearsed; the stranded-row sweep passed.
> The evidence is `docs/reviews/048-test-db-apply-record.md`. **It is NOT applied to prod; no prod GO exists.** Stale statements below carry their own dated strikethrough (the header line, the S1 rows, section 8.4, section 11, F4, U-8); the pinned text at `0b789e4` is unchanged and can be read with `git show`.
...
| **Artefact commit — everything below is pinned to this** | **Two pins, because a file cannot contain its own commit id.** **(1) The held SQL, the scaffold and both JSON files: `ae06082bd733419175efe09be6d01ac7e598fa71`** — the commit whose clean tree every capture in section 7 ran at. **(2) This package, the runbook, `docs/build-status.md`: `0b789e4ec358f205f2485438107e9ff47b1926f6`**, a commit that **contains this package file** (the package at that commit differs from this text only in the two places the pin is written — this row and the section 3 heading). The earlier pin `faaa8d4` **predates the package, the runbook and `down.sh`** (the package first appears at `f26a533`), so its own `git show faaa8d4:<path>` instruction failed for them — observed, logged: `fatal: path 'docs/reviews/048-review-package.md' exists on disk, but not in 'faaa8d4…'`. (R3-S2.) **At the moment the apply record is written, re-pin to the PR head of that moment and fetch it before citing it; the pinned commit must contain the package file.** Resolution was checked locally before the push; whether GitHub serves the new pin is checked after the push and recorded in the pass log — it cannot be asserted in advance. **DATED NOTE (2026-09-20, apply record written):** the apply record's evidence is pinned to the PR head **`fd96199600ec3325a8150a855fd4971b60918575`**, which **contains this package file** (blob `25b7a53fe3c5912c023538c905d6f2ac2b881e4a`) and **is served by GitHub** (`gh api …/contents/<path>?ref=fd96199…` returned matching blob ids for the migration, package, runbook and types — checked after the push). Since the `0b789e4` pin this package differs **only** by the two pin-writing places (commit `67b6e3b`: this row and the section 3 heading) and the dated corrections and checklist fill of 2026-09-20 (checked by diff); the certified text at `0b789e4` is unchanged and readable with `git show`. |
...
|---|---|---|
| **S1** typo-repair runbook | `docs/reviews/048-typo-repair-runbook.md` | ~~**Written, NOT rehearsed.** Rehearsal is a gate on the merge (section 4).~~ **DATED CORRECTION (2026-09-20): rehearsed on test-db** — `docs/reviews/048-test-db-apply-record.md` section 12 (two stated limits there). |
...
| **R3-N2** — the runbook's "why not delete" sentence | `docs/reviews/048-typo-repair-runbook.md` | **Fixed.** ~~The runbook is still **not rehearsed**~~ **(DATED CORRECTION 2026-09-20: rehearsed on test-db, `docs/reviews/048-test-db-apply-record.md` section 12.)** |
...
## 3. Artefacts (the SQL, the scaffold and the JSON pinned to `ae06082bd733419175efe09be6d01ac7e598fa71`; this package, the runbook and `docs/build-status.md` to `0b789e4ec358f205f2485438107e9ff47b1926f6` — section 0; view each with `git show <pin>:<path>`)
...
test-db apply, where the pass condition is that every teardown completes and leaves zero rows.~~ **DATED CORRECTION (2026-09-20): stale, and the claim was too strong.** That run has happened (run 35496136697 at `fd96199`, green, 119 files / 1350 tests): the sweep's `SELECT` and `DELETE` on `users.registered_by` ran **without error**, but the edge was **EMPTY** (0 populated rows; the only writer is the SQL function and no test calls it), so **recursion, child-first order and the cycle case remain UNOBSERVED. #69 is NARROWED, NOT CLOSED**; the first real observation will be the first engineer added through the add screen — `docs/reviews/048-test-db-apply-record.md` section 13. The reverse is also worth stating: this red run is evidence about *test-db's current schema*, not about the sweep's logic.
...
- **F4 — UNKNOWN #69 (the JS sweep has never run on a self-referential edge; this is the first of 34):** it is first
  **observed** at the first teardown that calls `sweepSharedFixtureReferences('users', …)` **on a test-db that carries
  ...
  observed: every teardown completes and leaves zero rows. **DATED CORRECTION (2026-09-20): the "first CI run after the apply" has happened (run 35496136697 at `fd96199`, green) and did NOT observe #69** — the sweep's `SELECT`/`DELETE` ran without error on an **empty** edge (0 populated rows; nothing calls the function), so recursion, child-first order and the cycle case are unobserved. **NARROWED, NOT CLOSED**; first observation = the first engineer added through the add screen (`docs/reviews/048-test-db-apply-record.md` section 13).
...
- ~~**No apply, anywhere; no DOWN rehearsal on test-db.** The DOWN block is **written and inert**, and executed only on the scaffold's disposable database (section 7). The full-tier DOWN rehearsal on the cleaned test-db, with captured output, is **owed**.~~ **DATED CORRECTION (2026-09-20): stale.** 048 **was applied to test-db** (`exfccwlrhoutkgrlikod`, 2026-09-20 06:34Z) and the DOWN **was rehearsed there** (`UP → DOWN → UP`, nine statements, both snapshots byte-identical) — `docs/reviews/048-test-db-apply-record.md` sections 1–4. **Prod was not touched.**
- ~~**The runbook is not rehearsed** (gate on the merge); R3-N2's wording is fixed.~~ **DATED CORRECTION (2026-09-20): stale.** The runbook **was rehearsed on test-db** (R1–R8, `docs/reviews/048-test-db-apply-record.md` section 12) — with two stated limits (the 3b query is not the roster code; `on_hold` keeps the engineer off the roster regardless).
...
- ~~**`types/database.ts` is not regenerated** — that is a post-apply step, once the columns exist on a database.~~ **DATED CORRECTION (2026-09-20): stale.** Regenerated from test-db after the apply and committed unedited as `fd96199` (it also carries the test-db-only helper `quoco_test_047_unused_rights_check`, accepted deliberately) — `docs/reviews/048-test-db-apply-record.md` section 8.
...
- [x] external review of this package: round 3 = **PACKAGE GO for the test-db apply** (folded items carried forward item-by-item: section 2b); no prod GO **(2026-09-20)** *[R — carried from this package's header and Aravind's instruction; not re-observed]* — `docs/reviews/048-test-db-apply-record.md` section 15.
- [x] freeze in force (no push after this pass's one push) — the PR head at apply time: **`67b6e3bb2754528ae448789e09d68e2ccf0c5c2e`** (origin) **(2026-09-20)**: the promotion commit `61f479e` was local and unpushed at the apply, and nothing was pushed between round 3's push and the apply (`docs/reviews/048-test-db-apply-record.md` section 0)
- [x] test-db apply: ref named, `project-ref` printed and equal; pre- and post-apply hash of the affected objects **(2026-09-20)** applied to `exfccwlrhoutkgrlikod`, 2026-09-20T06:34:23Z→06:34:28Z, CLI exit 0; project-ref printed and enforced in the apply frame; **pre-apply file sha256 `1f20efe1716c8fb0f0d18e856bfd3125e8d1559acfd761bde27cf51cd01a9873`**; post-apply hashes = F3's `md5_prosrc` / `md5_functiondef` (equal to the scaffold reference) — `docs/reviews/048-test-db-apply-record.md` sections 1–2.
- [x] fingerprint F1–F6 captured on test-db (section 5), and the two test-db-only observations (a) anon-key `42501` + `service_role` refused, (b) `authenticated` call succeeds **(2026-09-20)** F1–F6 each above its result, matching the scaffold reference (one benign difference: F5 `conkey` attnums, explained); (a) anon + service_role refused `42501`, 4 of 4, no `PGRST202`; (b) authenticated dry run HTTP 200 `{"applied": false, "rows": [{"idx": 0, "status": "ok"}]}`, nothing written (R3-U-2 resolved) — `docs/reviews/048-test-db-apply-record.md` sections 2, 6, 7.
- [x] **DOWN rehearsed on test-db with captured output**; `UP → DOWN → UP` **(2026-09-20)** nine statements (**not ten** — dated correction in the record); post-DOWN snapshot byte-identical to pre-apply (sha256 `48b64a3f9c009cd02c8983e3b166ca33d68992cbe40e3e45021790ca87ac9b6a`), post-re-UP byte-identical to post-apply (sha256 `697fc5954a2d92ba643e728fa1f5c32980264f1c0a84b0821bf55a2d4dd2e55b`); F1–F6 empty after DOWN — `docs/reviews/048-test-db-apply-record.md` section 4.
- [x] test-db carries 048 **for real** (`CLAUDE.md` §0(e)); ledger repaired **(2026-09-20)** `supabase migration repair --status applied 048 --linked`, exit 0; `migration list` shows 048 local and remote; 042/043/044/046 untouched — `docs/reviews/048-test-db-apply-record.md` section 5.
- [x] ~~test-db orphan cleanup (section 8.5) approved and run; `owner-deliver-job` re-run on the baseline registry = 21/21~~ **DONE 2026-09-19** (section 8.5's dated correction) — **replaced by:** stranded-row sweep by tenant id, done and printed **before** "fully green" is claimed **(2026-09-20)** **SWEEP DONE: 0 tenant rows and 0 user rows on the redesigned criterion (created at/after the Test job start `2026-09-20T07:10:10Z`, excluding the known-id list and the persistent outbound-send tenant), run `success` at `headSha` = `fd96199`** — `docs/reviews/048-test-db-apply-record.md` sections 10–11 (the known-tenant id list is in the repo there).
- [x] CI green on the PR head — **pinned run URL, `headSha` = PR HEAD**, **fully green** (section 8.3); every failure after apply is **new** **(2026-09-20)** **run 35496136697, `headSha` `fd96199600ec3325a8150a855fd4971b60918575`, `success`, 119 files / 1350 tests passed, 0 occurrences of the old error** — https://github.com/ara-2789/Quoco/actions/runs/35496136697 (`docs/reviews/048-test-db-apply-record.md` section 9). **Qualification:** pinned to `fd96199`; the PR head has since moved (the apply record's commit) and that commit's own run is not covered here — the merge needs a green run at the head at merge.
- [x] **typo-repair runbook rehearsed on test-db at `<path>`** — before the merge **(2026-09-20)** **rehearsed at `docs/reviews/048-test-db-apply-record.md` (section 12), dated 2026-09-20**: R1–R8, R3's natural red (1 row before the repair, 0 after), the number stays held (`already_on_this_project`), pinned cleanup. **Two stated limits:** the 3b query is the runbook's restatement of the roster predicate, not the roster code (T49 unbuilt); `on_hold` keeps the engineer off the roster regardless. Step 5 not exercisable on test-db.
...
- [x] backlog entries filed in `docs/build-status.md` (D12 rider, N1, R3-S1, R3-S3) — **filed in this PR**; line numbers in the entries' own headings **(2026-09-20)** verified present by grep of `docs/build-status.md`: index lines 40–43; entries at lines 401 (D12 rider), 450 (N1), 474 (R3-S1), 495 (R3-S3).
...
- [ ] migration file moved into `supabase/migrations/` **in the applying commit**, `types/database.ts` regenerated, file confirmed on `origin/main` (`git show origin/main:<path>`) **(2026-09-20)** **PARTIAL — left unticked:** moved into `supabase/migrations/` in `61f479e` (before the apply, in the same session; verified sha256-unchanged) and `types/database.ts` regenerated in `fd96199` — **but the file is not on `origin/main`** (not merged); this line closes only after `git show origin/main:supabase/migrations/048_engineer_registration.sql` succeeds.
- [x] `scripts/migration-number-reservations.json` updated **as part of the apply** (Step H) **(2026-09-20)** `d81031b` — **TEST-DB ONLY**: the entry says APPLIED TO TEST-DB ONLY and must be updated again at the prod apply.
- [x] a mistyped number stays held: no slice-1 path frees it (runbook step 4) **(2026-09-20)** runbook rehearsal R6: after the repair a dry run with the deactivated number returns `already_on_this_project`, not `ok` — `docs/reviews/048-test-db-apply-record.md` section 12.
...
8. **U-8 — #69, still unobserved.** The JS sweep on a self-referential edge; ~~the first observation is the first CI run after the test-db apply~~ **(DATED CORRECTION 2026-09-20: stale — that run happened, green, and did not observe it; the edge was empty. NARROWED, NOT CLOSED; the first observation is the first engineer added through the add screen — `docs/reviews/048-test-db-apply-record.md` section 13.)** A `registered_by` **cycle** (manual SQL only) would loop the sweep — inferred from reading, never run. **R3-N3 closes the length-1 case only** (section 9 item 9; U-16).
