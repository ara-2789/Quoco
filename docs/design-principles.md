...
Corollary: six questions per flow is a hard ceiling, not a preference — a count, not a mechanism. Below the ceiling, replace or piggyback is the preferred mechanism for new capture; append is the exception, spent deliberately, not the default. At the ceiling, append is no longer available at all — new capture must replace something already asked. Engineer-burden feature requests get rerouted to PM-side or system-side capture (the weather-API decision is the template).

DATED NOTE (2026-09-12, Aravind's decision, media-capture design pass — `docs/plans/media-capture-design.md`). This corollary previously read as an unconditional "never append," which the evening flow's new photo question (Q2, "Send today's site photos") knowingly violates — a conscious override, not an oversight, recorded here rather than left as a doc silently contradicted by the code. The corollary above is amended, not deleted, to say what is actually true now: six is a hard cap either way, append was always the one-time exception this project would eventually need to spend, and evening has now spent it. Evening is AT the ceiling — any future evening capture must replace an existing question, never append a seventh.
